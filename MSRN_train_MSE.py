...
parser.add_argument("--lambda_gan", type=float, default=0.5, help="Weight of deblurring loss, default=0.5")
...
training_settings=[
    {'nEpochs': 25, 'lr': 1e-4, 'step':  7, 'lr_decay': 0.5, 'lambda_db': 0.6, 'lambda_gan':0, 'gated': False},
    {'nEpochs': 60, 'lr': 1e-4, 'step': 30, 'lr_decay': 0.1, 'lambda_db': 0.5, 'lambda_gan':0.005, 'gated': False},
    {'nEpochs': 55, 'lr': 5e-5, 'step': 25, 'lr_decay': 0.1, 'lambda_db': 0.2, 'lambda_gan':0.005, 'gated': True}
]
...
def train(train_gen, model, netD, criterion, optimizer, epoch, lr):
    ...
    for iteration, batch in enumerate(train_gen):
        ...
        image_loss = opt.lambda_db * loss4 + loss1
        Loss_G = image_loss + loss_G_GAN * opt.lambda_gan
        epoch_loss += Loss_G
        optimizer.zero_grad()
        Loss_G.backward()
        optimizer.step()
...
for i in range(opt.start_training_step, 4):
    ...
    opt.lambda_gan = training_settings[i-1]['lambda_gan']
