...
def train(train_gen, model, criterion, optimizer, epoch, lr):
    ...
    for iteration, batch in enumerate(train_gen):
        ...
        if iteration % 200 == 0:
            textual_loss = 0
            perceptionloss = 0
            print("===> Epoch[{}]: image_loss:{:.4f}, text:{:.4f}, perception:{:.4f}, deblur:{:.4f}, sr:{:.4f}"
                  .format(epoch, image_loss.cpu(), textual_loss, perceptionloss, psnr_deblur, psnr_sr))

            f = open(FilePath, 'a')
            f.write(
                "===> Epoch[{}]: image_loss:{:.4f}, text:{:.4f}, perception:{:.4f}, deblur:{:.4f}, sr:{:.4f}, lr:{:.8f}"
                .format(epoch, image_loss.cpu(), textual_loss, perceptionloss, psnr_deblur, psnr_sr, lr) + '\n')
            f.close()
