...
from networks.Discriminator import Discriminator
from ESRGANLossMSE import GANLoss, VGGFeatureExtractor
...
def train(train_gen, model, criterion, optimizer, epoch, lr):
    ...
    for iteration, batch in enumerate(train_gen):
        ...
        if iteration % 200 == 0:
            ...
            print("===> Epoch[{}]: image_loss:{:.4f}, LossG:{:.4f}"
                  .format(epoch, image_loss.cpu(), Loss_G.cpu()))

            f = open(FilePath, 'a')
            f.write(
                "===> Epoch[{}]: image_loss:{:.4f}, LossG:{:.4f}, lr:{:.8f}"
                .format(epoch, image_loss.cpu(), Loss_G.cpu(),  lr) + '\n')
            f.close()
