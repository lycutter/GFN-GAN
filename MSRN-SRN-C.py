...
def train(train_gen, model, criterion, optimizer, epoch, lr):
    ...
    for iteration, batch in enumerate(train_gen):
        ...
        deblur_vgg, sr_vgg = deblurx32.clone(), sr.clone()
        ...
        # calculate loss_G
        # perception_deblur = cri_perception(deblurx32, Sharpx32)
        ...
        Loss_G = image_loss + (l1 + l2 + l3) * opt.lambda_db
        optimizer.zero_grad()
        Loss_G.backward()
        optimizer.step()
        epoch_loss += Loss_G
        ...
        if iteration % 200 == 0:
            ...
            print("===> Epoch[{}]:image_loss:{:.4f}, LossG:{:.4f}, perception:{:.4f}, text:{:.4f}"
                  .format(epoch, image_loss.cpu(), Loss_G.cpu(), perceptionloss, textual_loss))

            f = open(FilePath, 'a')
            f.write(
                "===> Epoch[{}]: image_loss:{:.4f}, LossG:{:.4f}, perception:{:.4f}, text:{:.4f}, lr:{:.8f}"
                .format(epoch, image_loss.cpu(), Loss_G.cpu(), perceptionloss, textual_loss, lr) + '\n')
            f.close()
