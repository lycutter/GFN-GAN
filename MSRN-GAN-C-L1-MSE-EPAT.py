...
import torch
from networks.MSRN_C_net import Net
import random
import re
...
parser.add_argument("--lambda_textual", type=float, default=0)
...
training_settings=[
    {'nEpochs': 10, 'lr': 1e-4, 'step': 5, 'lr_decay': 0.9, 'lambda_db':  0.5, 'gated': False},
    {'nEpochs': 20, 'lr': 5e-5, 'step': 5, 'lr_decay':  0.9, 'lambda_db': 0.5, 'gated': False},
...
def train(train_gen, model, criterion, optimizer, epoch, lr):
    ...
    for iteration, batch in enumerate(train_gen):
        ...
        perception_deblur = cri_perception(lr_deblur, LR_Deblur)
        image_deblur = criterion(lr_deblur, LR_Deblur)
        perception_sr = cri_perception(sr, HR)
        image_loss = criterion(sr, HR)
        perception_loss = opt.lambda_db * perception_deblur + perception_sr
        Loss_G = image_loss + image_deblur * opt.lambda_db + 0.02 * loss_G_GAN + textual_loss + 0.001 * perception_loss
        optimizer.zero_grad()
        Loss_G.backward()
        optimizer.step()
        epoch_loss += Loss_G
        psnr_sr = 10 * log10(1 / image_loss)
        psnr_deblur = 10 * log10(1 / image_deblur)
        if iteration % 200 == 0:


            print("===> Epoch[{}]: G_GAN:{:.4f}, image_loss:{:.4f}, LossG:{:.4f}, d_real:{:.4f}, d_fake:{:.4f}, perception:{:.4f}, textual:{:.4f}, deblur:{:.4f}, sr:{:.4f}"
                  .format(epoch, loss_G_GAN.cpu(), image_loss.cpu(), Loss_G.cpu(), d_loss_real.cpu(), d_loss_fake.cpu(), perception_loss, textual_loss, psnr_deblur, psnr_sr))

            f = open(FilePath, 'a')
            f.write(
                "===> Epoch[{}]: G_GAN:{:.4f}, image_loss:{:.4f}, LossG:{:.4f},  d_real_loss:{:.6f}, d_fake_loss:{:.6f}, lr:{:.8f}, perception:{:.4f}, textual:{:.4f}, deblur:{:.4f}, sr:{:.4f}"
                .format(epoch, loss_G_GAN.cpu(), image_loss.cpu(), Loss_G.cpu(), d_loss_real.cpu(), d_loss_fake.cpu(), lr, perception_loss, textual_loss, psnr_deblur, psnr_sr) + '\n')
            f.close()
